...
shop_list = list(shop_phone.keys())
...
# Shop Autocomplete
# -----------------------------
typed_shop = st.text_input("Shop Name", key="shop_name")

suggestions = [
    s for s in shop_list if typed_shop.lower() in s.lower()
][:5]

if suggestions and typed_shop != "":
    st.caption("Suggestions")
    selected = st.selectbox("Select Shop", [""] + suggestions)

    if selected:
        st.session_state.shop_name = selected
        st.session_state.contact_number = shop_phone[selected]

# Auto fill phone
contact_number = st.text_input(
    "Contact Number",
    value=st.session_state.get("contact_number",""),
    key="contact_number"
)
...
if submit_button:
    ...
    if typed_shop == "" or contact_number == "" or not valid_items:
...
if st.button("➕ New Order"):
    ...
    for key in st.session_state.keys():
        ...
        or key in ["shop_name", "contact_number","agent_name"]:
